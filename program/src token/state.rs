...
#[derive(Clone, Copy, Debug, Default, PartialEq)]
pub struct Account {
    /// The tyronzil DID associated with this account
    pub didtyronzil: Pubkey,
...
impl Pack for Account {
    ...
    fn unpack_from_slice(src: &[u8]) -> Result<Self, ProgramError> {
        ...
        let (did_suffix, mint, owner, amount, delegate, state, is_native, delegated_amount, close_authority) =
            array_refs![src, 32, 32, 32, 8, 36, 1, 12, 8, 36];
        Ok(Account {
            didtyronzil: Pubkey::new_from_array(*did_suffix),
    ...
    fn pack_into_slice(&self, dst: &mut [u8]) {
        ...
        let (
            ...
            close_authority_dst,
        ) = mut_array_refs![dst, 32, 32, 32, 8, 36, 1, 12, 8, 36];
        let &Account {
            ref didtyronzil,
            ref mint,
            ref owner,
        ...
        did_dst.copy_from_slice(didtyronzil.as_ref());
