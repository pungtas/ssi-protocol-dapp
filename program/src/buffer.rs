...
impl MessageBuffer {
    ...
    /// Unpacks a buffer
    pub fn unpack(input: &mut [u8]) -> Result<&mut MessageBuffer, ProtocolError> {
        if input.len() != std::mem::size_of::<MessageBuffer>() {
            Err(ProtocolError::MessageQueueAccountWrongSize)?;
        }
        ...
        if buffer.queue_head >= MESSAGE_HISTORY as u32 {
            Err(ProtocolError::MessageQueueBad)?;
        }
...
impl Message {
    pub fn unpack(input: &[u8]) -> Result<Message, ProtocolError> {
        ...
        if length as usize > MESSAGE_LENGTH {
            Err(ProtocolError::MessageLengthTooLarge)?;
        }

        if bytes[length as usize..].iter().any(|b| *b != 0) {
            Err(ProtocolError::MessageLengthTooSmall)?;
        }

        if length == 0 {
            Err(ProtocolError::MessageEmpty)?;
        }
...
mod tests {
    ...
    fn check_error(expected: ProtocolError, minput: Result<Message, ProtocolError>) {
        match minput {
    ...
    fn test_unpack_large() {
        ...
        check_error(ProtocolError::MessageTooLarge, Message::unpack(bytes));
    }
    ...
    fn test_unpack_small() {
        ...
        check_error(ProtocolError::MessageTooSmall, Message::unpack(&bytes[1..]));
    }
    ...
    fn test_unpack_short_length() {
        ...
        check_error(ProtocolError::MessageLengthTooSmall, Message::unpack(bytes));
    }
    ...
    fn test_unpack_empty() {
        ...
        check_error(ProtocolError::MessageEmpty, Message::unpack(bytes));
    }
}
